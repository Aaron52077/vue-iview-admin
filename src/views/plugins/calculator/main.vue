                    ...
                    <sForm ref="formValidate" :model="formObj" :rules="ruleValidate" :label-width="120">
                            ...
                            <sRow>
                                <sCol span="12">
                                    <sInputNumber
                                        ...
                                        placeholder="请输入贷款总额" /><span>万(商业)</span>
                                </sCol>
                                <sCol span="12" v-show="formObj.style === '2'">
                                    <sInputNumber
                                        ...
                                        placeholder="请输入贷款总额" /><span>万(公积金)</span>
                                </sCol>
                            </sRow>
                        </sFormItem>
                        ...
                        <sFormItem label="按揭年数" prop="year">
                            <Select v-model="formObj.year" placeholder="请选择按揭年数">
                                <Option v-for="item in yearList" :value="item.value" :key="item.value">{{item.key}}</Option>
                            </Select>
                        </sFormItem>
                         ...
                         <sFormItem label="贷款利率（%）" prop="rata">
                             <template v-if="formObj.style === '0'">
                                <sInputNumber 
                                    ...
                                    v-model="formObj.rate1" /> <span>%（商业贷款）</span>
                            </template>
                            <template v-if="formObj.style === '1'">
                                <sInputNumber 
                                    ...
                                    v-model="formObj.rate2" /> <span>%（公积金贷款）</span>
                            </template>
                            <template v-if="formObj.style === '2'">
                                <sInputNumber 
                                    ...
                                    v-model="formObj.rate3" /> <span>%（商业贷款）</span>
                                <sInputNumber 
                                    class="gc-loan__input"
                                    :min="0"
                                    :max="100"
                                    placeholder="请输入组合贷款利率" 
                                    v-model="formObj.rate4" /> <span>%（公积金贷款）</span>
                            </template>
                        </sFormItem>
                        <sFormItem>
                        ...
                        <sTimeline>
                            <sTimelineItem color="green">
                                ...
                                <template v-if="results.timeDate">
                                    ...
                                    <div v-show="results.overBasic">每月递减: {{(results || {}).overBasic || 0}}元</div>
                            ...
                            <sTimelineItem v-for="(item, index) in results.timeDate" :key="index">
                                <p>{{item.monthName}}：<span>{{formObj.type === '0' ? '本金' : '月供'}}：{{item.mouthBasic}}元</span>，<span>利息：{{item.mouthProfit}}元</span>，
                                <span>本期剩余：{{item.leftFund}}元</span></p>
                            </sTimelineItem>
                        </sTimeline>
                    </mScrollbar>
                </sCol>
...
export default {
	data() {
		return {
            ...
            ruleValidate: {
                ...
                money: [
                    { required: true, type: 'number', message: '请输入贷款总额', trigger: 'blur' }
                ],
                year: [
                    { required: true, message: '请选择按揭年数', trigger: 'change' }
                ],
                date: [
                    { required: true, type: 'date', message: '请选择首次还款日期', trigger: 'change' }
                ]
            },
            results: {}
		}
	...
	methods: {
                ...
                if (valid) {
                    ...
                    if (this.formObj.style === '0' || this.formObj.style === '1') {
                        ...
                        if (this.formObj.type === '0') {
                            resultInfo = this.calculateProfit(this.formObj.money, this.formObj.year, mouthVal, nowMouth, iv)
                        }
                        if (this.formObj.type === '1') {
                            resultInfo = this.calculateBasic(this.formObj.money, this.formObj.year, mouthVal, nowMouth, iv)
                        }
                    } else {
                        resultInfo = this.calculateGroup(this.formObj.money, this.formObj.fundmoney, this.formObj.year, mouthVal, nowMouth, iv3, iv4);
                    }
        ...
        hasDefVal(val1, val2) {
            return val1 ? (parseFloat((val1) / 100).toFixed(4)).toString() : val2
        },
        /**
         * 等额本息
        ...
        calculateProfit(num, year, mouth, time, iv) {
            ...
            // 每月月供
            const mouthLoan = (basicNum * mouthIV * Math.pow((1 + mouthIV), mouths)) / (Math.pow((1 + mouthIV), mouths) - 1);
            // 总利息 = 还款月数×每月月供额 - 贷款本金
            ...
            for (let i = 1; i <= mouths; i++) {
                ...
                const mouthProfit = basicNum * mouthIV * (Math.pow((1 + mouthIV), mouths) - Math.pow((1 + mouthIV), i - 1)) / (Math.pow((1 + mouthIV), mouths) - 1);
                // 每月应还本金 = 贷款本金 × 月利率 × (1 + 月利率)^(还款月序号 -1 ) ÷〔(1 + 月利率)^还款月数 - 1〕
                const mouthBasic = basicNum * mouthIV * Math.pow((1 + mouthIV), i - 1) / (Math.pow((1 + mouthIV), mouths) - 1);
        ...
        calculateBasic(num, year, mouth, time, iv) {
            ...
            let totalProfit = ((basicMouth + basicNum * mouthIV) + basicNum / mouths * (1 + mouthIV)) / 2 * mouths - basicNum;
            //还款总额 总利息+贷款本金
            let totalPrice = totalProfit + basicNum,
                leftFund = totalProfit + basicNum;
            // 循环月份
        ...
        calculateGroup(num1, num2, year, mouth, time, iv1, iv2) {
            ...
            let mergeTimeDate = profitObj.timeDate.map((item, index) => {
                return {
                    ...
                    mouthProfit: Number(parseFloat(item.mouthProfit) + parseFloat(basicObj.timeDate[index].mouthProfit)).toFixed(2),
                    mouthBasic: Number(parseFloat(item.mouthBasic) + parseFloat(basicObj.timeDate[index].mouthBasic)).toFixed(2),
                    leftFund: Number(parseFloat(item.leftFund) + parseFloat(basicObj.timeDate[index].leftFund)).toFixed(2) 
                }
            });
            return {
