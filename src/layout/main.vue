<template>
  <div class="gc-main container" :style="styleLayoutMainGroup" :theme="themeActive.name">
    ...
    <div class="gc-body">
      <div
        class="gc-body__lt sidebar-container"
        :style="{ width: dataBase.h5 ? `${layoutWeb}px` : '' }"
        :class="{ 'gc-h5': dataBase.h5 }"
      >
        <sidebarView />
      </div>
      <mInfiniteScroll
        class="gc-body__rt"
        :style="{ paddingLeft: dataBase.h5 ? `${layoutWeb}px` : '' }"
...
export default {
  ...
  computed: {
    ...
    ...mapGetters(["offsetTop", "cachedViews"]),
    layoutWeb() {
